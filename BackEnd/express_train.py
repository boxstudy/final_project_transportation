...
class ExpressTrain(Transportation):
    ...
    def __count_distance(self, place1, place2, file):
        ...
        with get_db_connection(self.data_path + file) as conn:
    ...
    # 找尋車站在哪個表之中
    def __find_table(self, station_name):
    ...
    def __check_route_direction(self, file, start_station, end_station):
        with get_db_connection(self.data_path + file) as conn:
            with conn.cursor() as cursor:
    ...
    def __create_path(self):
        ...
        files1= self.__find_table(self.start)
        files2 = self.__find_table(self.end)
        ...
        if file_set in transfer_points:
            ...
            if not self.__check_route_direction(file1, self.start, transfer_station):
                file1 = reverse_direction[file1]
            if not self.__check_route_direction(file2, transfer_station, self.end):
    ...
    def __create_time(self):
        if len(self.paths[0]) == 1:  # 不需轉車
            fastest_train, cheapest_train = self.__find_best_train(self.paths[0][0]["file"], self.start, self.end)
            ...
            # 取得第一段所有列車
            first_leg_fastest_train, first_leg_cheapest_train = self.__find_best_train(first_leg_file, self.start, transfer_station)

            # 取得所有可銜接的第二段列車(最快)
            second_leg_fastest_train, _ = self.__find_best_train(second_leg_file, transfer_station, self.end, first_leg_fastest_train["arrival_time"])

            # 取得所有可銜接的第二段列車(最便宜)
            _, second_leg_cheapest_train = self.__find_best_train(second_leg_file, transfer_station, self.end, first_leg_cheapest_train["arrival_time"])
    ...
    def __find_best_train(self, db_file, departure_station, arrival_station, min_departure_time=None):
    ...
    def __create_cost(self):
        for path_i in range(len(self.paths)):
            for route_i in range(len(self.paths[path_i])):
                ...
                with get_db_connection(self.data_path + route["file"]) as conn, conn.cursor() as cursor:
                    cursor.execute("""
                                    SELECT 
                                        (SELECT 距離 FROM train WHERE 車站 = ?) AS 出發距離,
                                        (SELECT 距離 FROM train WHERE 車站 = ?) AS 到達距離
